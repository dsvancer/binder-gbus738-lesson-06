...
Important function from this package include `conf_mat()`, `sens()`, `spec()`, `roc_curve()`, and `roc_auc()`
...
From the results below, we have a sensitivity of 0.76 on our test data results.
...
From the results below, we have a sensitivity of 0.78 on our test data results.
...
The ROC curve is a way to visualize the performance of any classification model. The plot includes the [sensitivity](https://en.wikipedia.org/wiki/Sensitivity_and_specificity){target="_blank"} on the y-axis and (1 - [specificity](https://en.wikipedia.org/wiki/Sensitivity_and_specificity){target="_blank"} on the x-axis for all possible probability cut-off values.
...
In the code below we create a new function, `my_metrics()` that will calculate the accuracy, sensitivity, specificity, F1, and ROC AUC from the results data frame.
...
To obtain the metrics on the test set we use `collect_metrics()`. Based on area under the ROC curve, our model has an "A+".
